...
def run_single_inference(cfg_path, exp_cfg, sd_model, idx, prompt):
    """
    ...
    prompt_embed = sd_model.load_prompt(exp_cfg["load_prompt_embeds"], prompt)
    ...
    if exp_cfg["model_identifier"] == "txt2img":
        ...
        idx_path = f'YOUR DATA PATH/laion-5B/data/{idx}.jpg'
        image, mask = Image.open(idx_path).convert("RGB"), None
...
def run_random_walk(cfg_path, exp_cfg, sd_model):
    ...
    for direction in range(exp_cfg["walk_directions"]):
        ...
        noise_delta = exp_cfg["step_size"] * torch.empty_like(lat_noise).uniform_(-1, 1) #TO DO: investigate and record this!!!!
         ##Another images embedding
         ##apply noise to a focussed block
            ...
            if step == 'start' or step % 25 == 0:
                for batch_idx in range(len(images)):
                    utils.save_sd_results(
                        ...
                        image_embed=image_embeds[batch_idx][-1].cpu(),
                        direction = noise_delta[batch_idx][-1].cpu(),
                        prompt_change= prompt_delta[batch_idx][-1].cpu(),
                        image=images[batch_idx][-1],
...
# def run_interpolation(cfg_path, exp_cfg, sd_model, prompts, idx, num_steps):
#     """
...
#     if len(prompts) > 1:  # Interpolates between multiple prompt embeddings
#         print(f"Interpolating {len(prompts)} prompts.")
#         log_txt = "prompt"
#         prompt_embeds = []
#         for i in range(len(prompts) - 1):
#             prompt_embed_1 = sd_model.load_prompt(
#                 load_prompt_embeds=prompts[i],
#                 prompt=prompts[i]
#             )
#             prompt_embed_2 = sd_model.load_prompt(
#                 load_prompt_embeds=prompts[i + 1],
#                 prompt=prompts[i + 1]
#             )
...
#         lst_itm = int_idx // (num_steps + 2)
#         step = int_idx % (num_steps + 2)
#         if step == 0:
#             step = "start"
...
#             prompt_embeds[int_idx * prompt_stepper].to(torch.float16),
#             latent_noise[int_idx * noise_stepper].to(torch.float16),
#             image,
#             mask
#         )
...
#                 image_embed=image_embeds[batch_idx][-1].cpu(),
#                 image=images[batch_idx][-1],
#                 file_name=f"interpolation_lstitms-{lst_itm}_{lst_itm + 1}_step-{step}"
#             )
#             results[batch_idx].append(images[batch_idx][-1])
...
def run_interpolation(cfg_path, exp_cfg, sd_model, prompts, latents, idx, num_steps):
    """
    Runs an interpolation experiment for the given Stable Diffusion model.
    ...
    # Loads the latent noise
    if len(latents) > 1:  # Interpolates between multiple latent noise embeddings
        print(f"Interpolating {len(latents)} latent gaussian noise tensors.")
        log_txt += " and latent noise" if log_txt else "latent noise"
        latent_noise = []
        for i in range(len(latents) - 1):
            noise_1 = sd_model.load_noise(
                load_latent_noise=latents[i],
                height=exp_cfg["height"],
                width=exp_cfg["width"],
                images_per_prompt=1,
                rand_seed=latents[i]
            ...
            noise_2 = sd_model.load_noise(
                load_latent_noise=latents[i + 1],
                height=exp_cfg["height"],
                width=exp_cfg["width"],
                images_per_prompt=1,
                rand_seed=latents[i + 1]
            )
            latent_noise += utils.interpolate(
                x=noise_1,
                y=noise_2,
        ...
        noise_stepper = 1
    else:  # Loads a single latent noise tensor from config file
        latent_noise = sd_model.load_noise(
            load_latent_noise=exp_cfg["inter_noises"][0],
            height=exp_cfg["height"],
            width=exp_cfg["width"],
